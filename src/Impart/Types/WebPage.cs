...
namespace Impart
{
    ...
    public class WebPage
    {
        ...
        {
            return $"<!-- Generated by Impart - https://github.com/PixelatedLagg/Impart -->{Environment.NewLine}<!DOCTYPE html>{Environment.NewLine}<html xmlns=\"http://www.w3.org/1999/xhtml\">{Environment.NewLine}    <link rel=\"stylesheet\" href=\"{cssPath}\">{Environment.NewLine}    <body>{textBuilder.Replace("%^", $"{Environment.NewLine}    ").ToString()}{Environment.NewLine}    </body>{Environment.NewLine}</html>";
        }
    }
}